...
if [ "$OS_VARIANT" = "Ubuntu-18.04" -o "$OS_VARIANT" = "Ubuntu-18.10" ]; then
  ...
  exit 0
elif [ "$OS_VARIANT" = "Ubuntu-20.04" ]; then
  echo "BUILDING $OS_VARIANT"
  prepare_and_build_debian "$PACKAGEDIR/$PACKAGE_NAME/ubuntu_20" $BUILD_NAME $PACKAGE_VERSION-$BUILD_NUMBER $INSTALL_ARTIFACTS "$OS_VARIANT" "$ARTIFACT_REPOSITORY"
  add_buildlog_information "$BUILD_NAME" "$PACKAGE_VERSION-$BUILD_NUMBER" "$BUILD_LOG"
  exit 0
