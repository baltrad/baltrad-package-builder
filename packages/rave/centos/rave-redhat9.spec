...
Requires: ((proj-devel >= 9.6 and proj-devel < 9.8) or proj96-devel or proj97-devel)
...
%build
make distclean || true
%configure --prefix=/usr/lib/rave --with-hlhdf=/usr/lib/hlhdf --with-proj=/usr/include,/usr/lib64 --with-expat --with-bufr=no --with-netcdf=yes  --enable-py3support --with-py3bin=python3.9 --with-py3bin-config=python3.9-config --with-python-makefile=/usr/lib64/python3.9/config-3.9m-x86_64-linux-gnu/Makefile
make
...
%py_byte_compile %{__python3} %{buildroot}/usr/lib/rave/Lib || :
...
%{_prefix}/include/*.h
/usr/lib64/python3.9/site-packages/rave.pth
